...
class _TotalAmountDetailPageState extends State<TotalAmountDetailPage> {
  ...
  void initState() {
    super.initState();
    _initializeMonths();
    final initialMonthKey =
        DateFormat('yyyy-MM').format(widget.initialFocusedMonth);
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _scrollToFocusedMonth());
  }
  ...
  void _initializeMonths() {
    ...
    if (widget.transactions.isEmpty) {
      ...
      DateTime minDate = widget.transactions
          .map((t) => t.date)
    ...
    while (
        currentMonth.isBefore(DateTime(maxMonth.year, maxMonth.month + 1, 1))) {
  ...
  Widget build(BuildContext context) {
    ...
    for (var t in widget.transactions) {
      ...
      if (monthlyTagData.containsKey(monthKey)) {
        monthlyTagData[monthKey]![t.tag] =
            (monthlyTagData[monthKey]![t.tag] ?? 0) + t.amount;
      }
    }

    final List<String> sortedMonthKeys = monthlyTagData.keys.toList()..sort();
    double maxY = monthlyTagData.values
        .map((e) => e.values.fold(0, (a, b) => a + b))
        .fold(0, (prev, val) => val > prev ? val.toDouble() : prev.toDouble());
    ...
    for (int i = 0; i < sortedMonthKeys.length; i++) {
      ...
      for (var tag in widget.allTags) {
        if (tagDataForMonth.containsKey(tag)) {
          ...
          final colorIndex =
              widget.allTags.indexOf(tag) % widget.tagColors.length;
          rodStackItems.add(
            BarChartRodStackItem(
                currentFromY, toY, widget.tagColors[colorIndex]),
          );
          currentFromY = toY;
        }
      }
            ...
            children: [
                                ...
                                barTouchData: BarTouchData(
                                  ...
                                  touchTooltipData: BarTouchTooltipData(
                                    ...
                                    getTooltipItem:
                                      ...
                                      final tagDataForMonth =
                                          monthlyTagData[monthKey] ??
                                              const <String, int>{};
                                      final totalForMonth = tagDataForMonth
                                          .values
                                          .fold<int>(0, (a, b) => a + b);
                                  ...
                                  touchCallback: (FlTouchEvent event,
                                    ...
                                    if (response?.spot != null &&
                                      ...
                                      if (touchedBarGroupIndex <
                                          sortedMonthKeys.length) {
                                        final selectedMonthKey =
                                            sortedMonthKeys[
                                                touchedBarGroupIndex];
                                        final selectedMonth = DateTime.parse(
                                            '$selectedMonthKey-01');
              ...
              Row(
                ...
                children: [
                  ...
                  Text(DateFormat('yyyy年MM月').format(_focusedMonth),
              ...
              const SizedBox(height: 16.0),
              Text('${DateFormat('yyyy年MM月').format(_focusedMonth)} の内訳',
                  style: const TextStyle(
                      fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8.0),
              Builder(
                builder: (context) {
                  final focusedMonthKey =
                      DateFormat('yyyy-MM').format(_focusedMonth);
                  final tagDataForMonth = monthlyTagData[focusedMonthKey] ?? {};
                  final totalForMonth = tagDataForMonth.values
                      .fold(0, (prev, amount) => prev + amount);
                  final sortedTags = tagDataForMonth.keys.toList()
                    ..sort((a, b) =>
                        tagDataForMonth[b]!.compareTo(tagDataForMonth[a]!));
